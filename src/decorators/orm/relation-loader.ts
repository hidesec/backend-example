import { Pool } from "pg";
import {
  EntityMetadata, RelationInfo, getEntityMetadata, getAllEntities,
...
export class RelationLoader<T extends object> {
  constructor(
    ...
    private readonly entityCtor: new (...args: any[]) => T
  ) {}
  ...
  /**
   * Mengisi array of raw row dengan relasi yang diminta.
   * Contoh: await loader.attach(rows, ["orders", "profile"]);
   * Hasilnya: tiap row dapat property tambahan sesuai nama relasi,
   * berisi raw row (atau array raw row) dari tabel terkait.
   */
  async attach(rows: RawRow[], relationNames: string[]): Promise<void> {
    ...
    for (const relName of relationNames) {
      ...
      if (!relation) {
        console.warn(`Relation "${relName}" not found on ${this.entityCtor.name}`);
        continue;
      }
      ...
      switch (relation.type) {
        case "ManyToOne":
        case "OneToOne":
          await this.attachToOne(rows, relation);
          break;
        case "OneToMany":
          await this.attachOneToMany(rows, relation);
          break;
        case "ManyToMany":
          await this.attachManyToMany(rows, relation);
          break;
      }
    }
  }
  ...
  private async attachToOne(rows: RawRow[], relation: RelationInfo): Promise<void> {
    ...
    const result = await this.pool.query(`SELECT * FROM ${qualifiedTarget} WHERE id = ANY($1)`, [ids]);
    const byId = new Map(result.rows.map((r) => [r.id, r]));

    rows.forEach((r) => {
      r[relation.propertyName] = byId.get(r[fk.columnName]) ?? null;
    });
  }
  ...
  private async attachOneToMany(rows: RawRow[], relation: RelationInfo): Promise<void> {
    ...
    if (!inverseFk) {
      console.warn(`Cannot resolve inverse FK for OneToMany "${relation.propertyName}"`);
      return;
    }
    ...
    const result = await this.pool.query(
      `SELECT * FROM ${qualifiedTarget} WHERE ${inverseFk.columnName} = ANY($1)`,
      [parentIds]
    );
    ...
    rows.forEach((r) => {
      r[relation.propertyName] = grouped.get(r.id) ?? [];
    });
  }
